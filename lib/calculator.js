...
class Calculator {
   ...
   */
  div(v) {
    this._vi = this._vi / Number(v)
    const s = this._vi.toString()
    const pe = s.indexOf('e')
    if (pe < 0) {
      const pos = s.indexOf('.')
      if (pos > 0) {
        this._vi = Number(s.substr(0, pos) + s.substr(pos + 1))
        this._p = this._p + s.length - pos - 1
      }
    } else {
...
function compile(expr, ...names) {
  return (new Function('$', ...names, 'return ' + build(expr) + '.v()')).bind(Calculator, function (v) {
    return new Calculator(v)
  })
}
